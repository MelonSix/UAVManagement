...
public class ControlCenter implements KnowledgeAwareInterface {
    ...
    private int sub_team_size;

    public Map<Integer, Set<Integer>> locked_threat;

    private static Logger logger = (Logger) LoggerFactory.getLogger(ControlCenter.class);

    /** constructor of the control center.
     * 
     * @param kb is the knowledge of the control center
     */
    public ControlCenter(KnowledgeInterface kb, int sub_team_size) {
        this.kb = kb;
        this.sub_team_size = sub_team_size;
        //ensures that at least an attacker is assigned to a threat
        if(this.sub_team_size<1)
            this.sub_team_size=1;
